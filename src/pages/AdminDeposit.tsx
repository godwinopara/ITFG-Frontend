...
import { columnAdminDeposit, columnDeposit, columnUsers } from "../components/dashboards/TableCol";
...
const AdminDeposit = () => {
  ...
  const [usersDeposits, setUsersDeposits] = useState<TransactionProps[]>([]);
  ...
  const fetchUsersDeposits = async () => {
    try {
      ...
      setUsersDeposits(deposits);
      dispatch({ type: "GET_USERS_DEPOSITS", payload: deposits });
    } catch (error) {}
  };
      ...
      {loading ? (
        ...
        <>
          <div className="flex justify-between items-center">
            <h1 className="text-slate-600  font-maisonBold text-xl">Users Deposits</h1>
          </div>
          ...
          <div className="border rounded-t-[6px] p-4 mt-5">
            ...
            <p className="text-gray-600 text-sm"> A total of {usersDeposits.length} deposit(s)</p>
          </div>
          <div>
            <DataTable columns={columnAdminDeposit} data={usersDeposits || []} />
          </div>
        </>
      )}
    </AdminLayout>
