...
import { getAllEndedInvestments } from "../api/api";
...
const AdminDeposit = () => {
  ...
  const [usersEndedInvestments, setUsersEndedInvestments] = useState<InvestmentProps[]>([]);
  ...
  const fetchUsersInvestments = async () => {
    ...
    try {
      const endedInvestments = await getAllEndedInvestments();
      setUsersEndedInvestments(endedInvestments);
      dispatch({ type: "GET_USER_ENDED_INVESTMENTS", payload: endedInvestments });
    } catch (error) {
      console.error(error);
    } finally {
      ...
      {loading ? (
        ...
        <>
          <div className="flex justify-between items-center">
            <h1 className="text-slate-600  font-maisonBold text-xl">Users Ended Investments</h1>
          </div>
          ...
          <div className="border rounded-t-[6px] p-4 mt-5">
            ...
            <p className="text-gray-600 text-sm"> A total of {usersEndedInvestments.length} Ended Investment(s)</p>
          </div>
          <div>
            <DataTable columns={columnsAdminInvestment} data={usersEndedInvestments || []} />
          </div>
        </>
      )}
    </AdminLayout>
