...
import { getAllReferrals } from "../api/api";
...
const AdminReferralList = () => {
  ...
  const [usersDeposits, setUsersDeposits] = useState<AdminReferralProps[]>([]);
  ...
  const fetchAllReferrals = async () => {
    try {
      ...
      setUsersDeposits(referrals);
      console.log(referrals);
      dispatch({ type: "GET_USERS_REFERRALS", payload: referrals });
    } catch (error) {
      console.log(error);
    }
  };
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="border rounded-t-[6px] p-4 mt-5">
            ...
            <p className="text-gray-600 text-sm"> A total of {usersDeposits.length} referrals(s)</p>
          </div>
          <div>
            <DataTable columns={columnsAdminReferral} data={usersDeposits || []} />
          </div>
        </>
      )}
    </AdminLayout>
