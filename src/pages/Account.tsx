...
const Account = (props: Props) => {
  ...
  return (
    <AdminLayout>
      <Modal show={showModal} closeModal={closeModal} title="Update Balance" width={500} height={500}>
        <form onSubmit={handleSubmit}>
          <div className="mb-5 mt-10">
            <label className="mb-2.5 block font-medium text-white">Account Balance</label>
          ...
          <div className="mb-5">
            <label className="mb-2.5 block font-medium text-white">Profit</label>
            <div className="relative">
              <input
                type="text"
                name="profit"
      ...
      <div className="rounded-sm border   px-5 pt-6 pb-2.5 shadow-default border-strokedark bg-boxdark sm:px-7.5 xl:pb-1">
        <h2 className="font-bold text-xl mb-5  p-4 text-white rounded-md ">ALL USERS BALANCE</h2>
          ...
          <table className="w-full table-auto">
            <thead>
              <tr className="text-left bg-meta-4">
                ...
                <th className="min-w-[100px] py-4 px-4 font-medium text-white dark:text-white">Balance</th>
                <th className="min-w-[100px] py-4 px-4 font-medium text-white dark:text-white">Profit</th>
                <th className="min-w-[100px] py-4 px-4 font-medium text-white dark:text-white">Bonus</th>
            ...
            <tbody>
              {paginatedUsers?.map((userItem: any, key: number) => (
                <tr key={key}>
                  ...
                  <td className="border py-5 px-4 border-strokedark">
                    <p className="text-white dark:text-white">${userItem?.balance}</p>
                  </td>
                  <td className="border py-5 px-4 border-strokedark">
                    <p className="text-white dark:text-white">${userItem.profit}</p>
                  </td>
                  <td className="border py-5 px-4 border-strokedark">
                    <p className="text-white dark:text-white">${userItem.bonus}</p>
                  </td>

                  <td className="border py-5 px-4 flex items-center gap-x-2 border-strokedark">
                    <button
                      onClick={() =>
                        handleClickToggleModal(userItem.uid || "", userItem.profit, userItem.balance, userItem.bonus)
                      }
