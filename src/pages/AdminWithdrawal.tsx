...
import { columnAdminWithdrawal } from "../components/dashboards/TableCol";
...
const AdminWithdrawal = () => {
  ...
  const [usersWithdrawals, setUsersWithdrawals] = useState<TransactionProps[]>([]);
  ...
  const fetchUsersDeposits = async () => {
    try {
      ...
      setUsersWithdrawals(withdrawals);
      dispatch({ type: "GET_USERS_WITHDRAWALS", payload: withdrawals });
    } catch (error) {}
  };
      ...
      {loading ? (
        ...
        <>
          <div className="flex justify-between items-center">
            <h1 className="text-slate-600  font-maisonBold text-xl">Users Withdrawal</h1>
          </div>
          ...
          <div className="border rounded-t-[6px] p-4 mt-5">
            <h2 className="font-maisonBold mb-1 text-primary text-base">Users Withdrawals</h2>
            <p className="text-gray-600 text-sm"> A total of {usersWithdrawals.length} withdrawal(s)</p>
          </div>
          <div>
            <DataTable columns={columnAdminWithdrawal} data={usersWithdrawals || []} />
          </div>
        </>
      )}
    </AdminLayout>
